...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "SELECT * FROM penyelia WHERE email = ?";
    ...
    if ($result && $result->num_rows == 1) {
        ...
        // Kalau password tetap "uthm" seperti anda guna sebelum ini
        if ($password === 'uthm') {
            session_regenerate_id(true);
            $_SESSION['user_id'] = $user['id_penyelia'];
            $_SESSION['name'] = $user['nama_penyelia'];
            $_SESSION['role'] = 'penyelia';

            header("Location: supervisor_dashboard.php");
            exit();
        } else {
            $error = "Kata laluan salah. Gunakan 'uthm'.";
        }
    } else {
...
<meta name="viewport" content="width=device-width, initial-scale=1.0">
  ...
  <meta charset="UTF-8" />
  <title>Login Penyelia - FSKTM FYP</title>
<style>
...
<body>
  ...
  <div class="container">
    <h2>Log Masuk Penyelia</h2>
    <?php if ($error) echo "<div class='error'>" . htmlspecialchars($error) . "</div>"; ?>
    <form method="post" action="">
      <div class="form-group">
        <label for="email">Email Penyelia:</label>
